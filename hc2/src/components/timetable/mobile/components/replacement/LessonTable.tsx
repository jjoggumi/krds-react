...
export interface LessonTableProps {
  className?: string;
  /** 선택된 교사 ID, 값이 있으면 해당 교사의 수업만 표시 */
  ...
  selectedCell?: { lessonDate: number; period: number } | null;
...
export function LessonTable({ 
  ...
  useEffect(() => {
    ...
    const maxPeriodFromConfig = timetableConfig.maxPeriod || minimumPeriod;
    setMaxPeriod(Math.max(minimumPeriod, maxPeriodFromConfig)); // 수업
                ...
                <tr key={`period-row-${periodIdx}`}>
                    ...
                    {lessonDays.map((d, dayIdx) => (                      
                      <LessonCell
                        ...
                        isEmptySelected={
                          !dailyLessonOfCell(d.lessonDate, periodIdx + 1) &&
                          selectedCell?.lessonDate === d.lessonDate &&
                          selectedCell?.period === periodIdx + 1
                        }
                      />
                    ))}
                </tr>
