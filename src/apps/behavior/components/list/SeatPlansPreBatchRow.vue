    ...
    <ul :class="['row-type', addEaClassName]">
      <li v-for="item of item.seatSections" :key="`seat-sections-${item.sectionId}`">
        ...
        <ul class="plan-student">
          <li v-for="seat of item.seats" :key="`seat-${seat.seatId}-${seat.student ? seat.student.studentId : getSeatKey(index)}`"
            :class="{
                fixed: seat.isFixed
            }"
          >
          ...
          async randomPositionBatch() {
            ...
            if(this.item.sectionType === "SINGLE_COLUMN") { // 분단 1열 
              ...
              if(this.item.pairingType === "RANDOM") {    // 랜덤 
                ...
                if(this.item.avoidPreviousPartner) {    // 이전짝궁 
