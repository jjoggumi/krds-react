<template>
  <div class="plan-type" ref="planType">
    <div class="plan-type-inner" ref="planTypeInner">
      ...
      <ul :class="['row-type', addEaClassName]">
        ...
        >
          ...
          <ul class="plan-student">
            <li v-for="seat of item.seats" :key="`seat-${seat.seatId}}`"
              :class="{
                fixed: seat.isFixed,
              }"
            >
...
export default {
  ...
  computed: {
    ...
    addEaClassName() {
      ...
      if(this.item.sectionType === "SINGLE_COLUMN") {
        ea = `${ea}1${this.item.sectionCount}`
      } else {
        ea = `${ea}2${this.item.sectionCount}`
      }

      let addCount = 0
      if(this.item.seatSections) {
        addCount = ((this.item.seatSections || [])[0] || {}).seatCount || 0
        if(addCount >= 10) {
          addCount = addCount / 2
        }
      }
    ...
    isHidden() {  // 버전별로 보여지는 변경사항에 따라 변경, 없어도 되는건데 이후 값이 필요할수있어서 남겨둠 
  ...
  methods: {
    ...
    async autoPositionBatch() {
      ...
      let students = await this.getClassroomStudents({classroomId: this.classroomId, isHidden: false, isIncludePoint: true})
      const studentCount = students.length
      this.$refs.startAlert.classList.remove('on-random', 'on-onece')

      const sumSeatsFor = filterFunc => this.item.seatSections.reduce((acc, section) => 
                          acc + section.seats.filter(filterFunc).length, 0)

      let activeCount = sumSeatsFor(v => v.isActive)
      let activeCount2 = sumSeatsFor(v => v.isActive && v.student)
      let seatCount = sumSeatsFor(v => v.isActive)
      let list = this.item.seatSections.map(v => v.seats)
      let batchedCount = sumSeatsFor(v => v.student)
      let isBatched = batchedCount > 0

      const maximumSeatCount = this.item.sectionType === "SINGLE_COLUMN" ? 48 : 56
      const checkDiff = (maximumSeatCount => {
        return seatCount !== studentCount 
          && (studentCount === 0 || activeCount > studentCount 
          || (seatCount < maximumSeatCount && studentCount > seatCount))
      })(maximumSeatCount)
      ...
      if(seatCount === maximumSeatCount) {
        students = students.slice(0, seatCount)
      }
      ...
      if(ramainCount > 0) {
        if(isBatched) {
          ...
          for(const seats of list) {
            for(const seat of seats) {
              if (seat.isActive && seat.student) continue
              seat.isActive = true
      ...
      if(this.item.sectionType === "SINGLE_COLUMN") { // 분단 1열 
        this.shuffleArray(students)
        
        if(isBatched) {
          const activeSeats = availSeats.filter(v => v.isActive)
          const seatList = activeSeats.filter(v => !v.isFixed)
          const seatStudentIds = seatList.map(v => v.student ? v.student.studentId : null)
          const studentList = [...students.filter(s => seatStudentIds.includes(s.studentId))]

          studentSeats = [
            ...activeSeats.filter(v => v.isFixed),
            ...seatList.map((v, i) => ({ ...v, student: {...studentList[i]} })),
            ...availSeats.filter(v => !v.isActive)
          ]

          studentSeats = studentSeats.map(item => ({ ...item,
              studentId: (item.student || {studentId: null}).studentId }))
        } else {
          studentSeats = simpleMapping(availSeats, students)
        }
      } else if(this.item.sectionType === "DOUBLE_COLUMN") {    // 분단 2열 
        let studentList = []
        let pairSeatList = []
      ...
      if (!IMMEDIATE) this.$refs.startAlert.classList.add(this.item.viewMode === 'SINGLE_VIEW' ? 'on-random' : 'on-onece');
      ...
      setTimeout(
        ...
        IMMEDIATE ? 0 : this.item.viewMode === 'SINGLE_VIEW' ? 1100 : 3000
      );
    },
    genderToast() {
      setTimeout(() => {
    ...
    async sectionNameEdit(sectionId, e){
      e.stopPropagation()
