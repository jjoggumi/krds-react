    ...
    >
      <div class="modal-cont">
        <div class="modal-cont-inner">
          ...
          <!-- 메인영역-->
          <div v-show="!isFileLoading" class="pdf-viewer-layout">
...
export default {
  ...
  data() {
    return {
      ...
      isLoading: true,
      errorMessage: {
        alert: '미리보기를 지원하지 않는 파일입니다.<br>첨부파일을 다운로드 합니다.',
        confirm: '미리보기를 지원하지 않는 파일입니다.<br>첨부파일을 다운로드 하시겠습니까?',
      },
    ...
    showFlag() {
      if (!this.flag) {
        try {
          ...
          // 통합 후 삭제 예정
          this.$emit('closeDocView', this.flag)
        } catch (error) {
  ...
  created() {
    ...
    if (this.isAvailablePreview()) {
      this.filePath = this.file.fileConvertPath || this.file.fileOriginalPath
    } else {
      this.flag = false
      this.$hiClass.alert(this.errorMessage.alert)
          .then(() => {
            this.$comn.downloadFile(this.file)
          })
    }
  },
  mounted() {
    this.$hiClass.toggleBodyClass('add', 'hidden')
    if (this.flag) this.getPdf().then()
  ...
  methods: {
    ...
    fileDownload() {
      this.$comn.download(this.file.fileOriginalPath, this.file.fileName)
    },
    isAvailablePreview() {
      ...
      const extension = this.$comn.split(this.file.fileOriginalPath, '.')
      ...
      switch (extensionLowerCase) {
        ...
        default: {
          isEnabled = enabledFileExt.includes(extensionLowerCase)
              && this.file.fileConvertPath
        }
      }
      return isEnabled
    },
    ...
    async getPdf() {
      ...
      try {
        ...
        if (err.message === 'Failed to fetch') {
          ...
          this.$hiClass.alert('파일이 손상되어 PDF 뷰어를 실행할 수 없습니다.'); //case 3: 손상된 pdf
