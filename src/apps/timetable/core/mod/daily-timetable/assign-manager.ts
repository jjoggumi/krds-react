...
export default abstract class DailyTimetableAssignManager {
  protected _totalRetryCount = 0;
  protected _context: DailyTimetableDataContext;
  ...
  assignLessonConfToPeriodOverview(
    ...
    lessonConf: LessonConf,
    periodOvervew: DailyPeriodOverview,
    consecutiveGroupId = '',
    isManuallyAssigned = false
  ): Lesson | undefined {
    ...
    // @TODO: 복수 교사이고, 이미 다른 교사의 수업이 이미 배정되어 있는 경우의 처리도 필요함.    
    ...
    remainingLessonPeriods.splice(findedLesson, 1);
  ...
  */
  ...
  assignLesson(lesson: DailyLesson): void {
    ...
    this.assignLessonToPeriodOverview(
      lesson,
      classPeriodOverviewMap[lesson.classId]
    );

    // 교사 배정 정보 처리
    lesson.lessonTeacherIds?.forEach((teacherId) => {
      const teacherPeriodOverview: TeacherDailyPeriodOverview =
        teacherPeriodOverviewMap[teacherId];
  ...
  assignLessonToPeriodOverview(
    ...
    const { lessonDate, dayOfWeek, period, courseId, classId } = lesson;

    const course = this._context.courseMap[courseId];
    const cls = this._context.classMap[classId];

    const lessonDailyPeriod = [lessonDate, period, dayOfWeek] as DailyPeriodTuple;
    ...
    const findedLesson = remainingLessonPeriods.findIndex(
      (period) => period[0] === lessonDailyPeriod[0] && period[1] === lessonDailyPeriod[1] // lessonDate와 period만 비교
    );
  ...
  getRemainingPeriodsByClassAndTeacher(
    ...
    const { classPeriodOverviewMap, teacherPeriodOverviewMap } = this._context;
    const periodTuples: DailyPeriodTuple[][] = [];
    ...
    const periods = TimetableUtils.intersectDailyPeriodTuples(...periodTuples);
  ...
  */
  ...
  resetLessonToPeriodOverview(lesson: DailyLesson, periodOverview: DailyPeriodOverview) {
    if (!lesson || !periodOverview) {
      return;
    }

    const { lessonDate, period, dayOfWeek } = lesson;
    const lessonPeriod = [lessonDate, period, dayOfWeek] as DailyPeriodTuple;
    ...
    const findedLessonConf = assignedLessonConfs.findIndex(
      (conf) =>
        ...
        lesson.lessonTeacherIds?.some((tId) => tId === conf.teacherId)
    );
    ...
    const [assignedLessonConf] = assignedLessonConfs.splice(
      findedLessonConf,
      1
    );
  ...
  resetAssignedLesson(lesson: DailyLesson) {
    const { classPeriodOverviewMap, teacherPeriodOverviewMap } = this._context;
    ...
    lesson.lessonTeacherIds?.forEach((tId) => {
      const teacherPeriodOverview = teacherPeriodOverviewMap[tId];
      this.resetLessonToPeriodOverview(lesson, teacherPeriodOverview);
    });
  }


  chainExchangeLessons(lessons: DailyLesson[]): void {
   ...
   */
  shiftLessonsToLeft(emptyPeriod: TimetablePeriod, lessons: DailyLesson[]): void {
    // 파라미터 검증: 수업 중 동시, 연속 수업 등이 있으면 이동 불가
    const isNotValidLessons = lessons.some(
      (lesson) => lesson.concurrentCourseId || lesson.consecutiveGroupId
    );
  ...
  getRemainingPeriodsByClassAndTeacherForPrint(
    ...
    // 출력을 위함
    const classPeriodTuples: DailyPeriodTuple[][] = [];
    const teacherPeriodTuples: DailyPeriodTuple[][] = [];
    ...
    // 출력
    const teacherIntersectPeriods = TimetableUtils.intersectDailyPeriodTuples(
      ...teacherPeriodTuples
    );
    const classIntersectPeriods = TimetableUtils.intersectDailyPeriodTuples(
      ...classPeriodTuples
    );
    ...
    teacherPeriodTuples.forEach((perds, idx) => {
      ...
      // 교사 별 배정 가능한 시간
      const resultPeriod = TimetableUtils.intersectDailyPeriodTuples(
        perds,
        ...classPeriodTuples
      );
