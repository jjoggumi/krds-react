 ...
 * 수업 배정을 관리한다
 */
export default class CommonLessonManager extends TimetableAssignManager {
  ...
  autoAssignWithLessonConfs(lessonConfs: LessonConf[]): AssignResult {
    ...
    if (assignPeriods.length === 0) {
      ...
      if (retryClassCount >= this._maxTryClassCount) {
        ...
        this._context.printLog(`[${this.constructor.name}]`,`배정 가능한 시간이 없습니다. (${cls.grade}0${cls.classNumber})`);
  ...
  findAssignPeriodsWithLessonConfs(
    ...
    lessonConfs.forEach((conf) => {
      ...
      if(course.isDoubleTeacher) {
        // 복수 교사 과목인 경우, 다른 교사도 포함한다.
        const partnerTeacherLessonConfs = this._context.lessonConfs.filter((lc) => {
          return lc.classId === conf.classId && lc.courseId === conf.courseId && lc.teacherId !== conf.teacherId;
        });

        partnerTeacherLessonConfs.forEach((lc) => {
          teacherIdSet.add(lc.teacherId);
        });
      }
    });
    ...
    if (!remainingPeriods || remainingPeriods.length === 0) { // 배정 가능한 시간이 없음
      return { assignPeriods: [] };
    }
  ...
  currentUnassignedLessonConfs(): LessonConf[] {
    ...
    */
    ...
    // remainingLessonConfs가 가장 적은 학급의 remainingLessonConfs를 반환
    const overview = periodOvervews
      .filter((overview) => overview.remainingLessonConfs.length > 0)
      .sort((a, b) => {
        return a.remainingLessonConfs.length - b.remainingLessonConfs.length;
      })[0];
