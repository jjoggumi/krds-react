  ...
  <div class="course-select" v-click-outside="resetInput">
    <label :for="`inputTag` + index" class="selected-tags" :class="{ isError }"  >
      ...
      <HiTag
        ...
        :tag="`${tag.isDoubleTeacher ? '(복) ' : ''}${tag.displayedTitle} (${tag.periodCount})`"
        :is-error="false"
      ...
      <span class="autocomplete-wrap">
        ...
        <div class="autocomplete-list custom-scr" v-if="isDropdownVisible">
          <button type="button" 
            ...
            :key="`dropdown-${tag.courseId}`"          
            @click="addTag(tag)"
            :disabled="isSelected(tag)"
          >
            {{ tag.displayedTitle }}({{ tag.standardCourseTitle }})
            <span class="txt-primary">({{ tag.periodCount }})</span>
          </button>
          <div
...
export default {
  ...
  methods: {
    ...
    // 선택된 태그인지 확인
    isSelected(tag) {
      return this.selected.some((selectedTag) => selectedTag.courseId === tag.courseId);
    },
